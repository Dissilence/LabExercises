...
const {randomAnime, randomAnimeCache} = require('../models/animeRecc')

const randomAnime= (req, res) => {
    ...
    axios.get('https://kitsu.io/api/edge/trending/anime') //gets a single random cat fact
        .then(response => {
...
const listAnime = (req, res) => {
    ...
    axios.get('https://kitsu.io/api/edge/trending/anime='+limit+'&page='+page) //gets a list of random cat facts
        .then(response => {
            //console.log(response.data)
            ...
            facts.forEach((fact, index) => { //index can be used in foreach to keep track of which array element we're iterating over
                ...
                randomAnimeCache.set(id, new randomAnime(id, anime.attributes.titles.en, anime.attributes.ratingRank))
            })
...
const getAnime = (req, res) => {
    ...
    let randomAnime = randomAnimeCache.get(parseInt(req.params.id))
    console.log(randomAnime);

    if (randomAnime) {
        res.status(200).json({success: true, ...randomAnime})
    } else {
        res.status(404).json({success: false, message: 'Anime #'+req.params.id+' not found in cache'})
    }
}
